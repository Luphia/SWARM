...
Bot.prototype.createTag = function (tag, cb) {
	...
	var newTag = {_id: new mongodb.ObjectID().toString(), name: '', type: '', ctime: now};
	for(var k in tag) {
	...
	if(newTag.files !== undefined) {
		this.checkFileExists(uid, newTag.files, function (e, d) {
			if(e) { return cb(e); }
			newTag.files = d;
			collection.insert(newTag, {}, function (_e, _d) {
				if(_e) { return cb(_e); }
				cb(null, newTag);
			});
		});
		...
		collection.insert(newTag, {}, function (e, d) {
			if(e) { return cb(e); }
			cb(null, newTag);
		});
	}
...
Bot.prototype.checkFileExists = function (uid, files, cb) {
	...
	files.map(function(v) {
		...
		else { try { search.push(v); } catch(e) {} }
	});
